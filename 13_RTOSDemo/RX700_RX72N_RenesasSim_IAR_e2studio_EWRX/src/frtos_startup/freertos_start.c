...
/* Locking time counter for the Command Console. */
static uint16_t usTicksCmdConsoleInUse = 0;
#if defined(RENESAS_SIMULATOR_DEBUGGING) && (RENESAS_SIMULATOR_DEBUGGING != 0)
/* Hardware or Renesas RX Simulator */
#define DEBUG_CONSOLE_LOCK_TICKS pdMS_TO_TICKS( !IsRenesasSimDebugMode() ? 10000 : 1 )
#else
/* Hardware only */
#define DEBUG_CONSOLE_LOCK_TICKS pdMS_TO_TICKS( 10000 )
#endif
...
void vApplicationTickHook(void)
{
    ...
    #if mainCREATE_SIMPLE_BLINKY_DEMO_ONLY == 0
    {
        ...
        {
            extern void vSerialPortTickHook( uint16_t *pusTicksCmdConsoleInUse );

            vSerialPortTickHook( &usTicksCmdConsoleInUse );
        }
        ...
        if( DEBUG_CONSOLE_LOCK_TICKS < usTicksCmdConsoleInUse )
        {
            usTicksCmdConsoleInUse = 0;
        }
        else if( 0 < usTicksCmdConsoleInUse )
        {
            usTicksCmdConsoleInUse++;
        }
    }
    #endif
...
{
    /* Ensure that the Debug Console isn't locked for the Command Console. */
    if( usTicksCmdConsoleInUse == 0 )
    {
