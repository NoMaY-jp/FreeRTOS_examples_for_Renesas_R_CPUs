...
uint8_t   g_iica1_master_status_flag;  /* iica1 master flag */
uint8_t   g_iica1_slave_status_flag;   /* iica1 slave flag */
uint8_t * gp_iica1_rx_address;         /* iica1 receive buffer address */
uint16_t  g_iica1_rx_len;              /* iica1 receive data length */
uint16_t  g_iica1_rx_cnt;              /* iica1 receive data count */
uint8_t * gp_iica1_tx_address;         /* iica1 send buffer address */
uint16_t  g_iica1_tx_cnt;              /* iica1 send data count */
...
static MD_STATUS U_IICA1_Slave_Send(const uint8_t *tx_buf, uint16_t tx_num);
static MD_STATUS U_IICA1_Slave_Receive(volatile uint8_t *rx_buf, uint16_t rx_num);
static MD_STATUS U_IICA1_Slave_Receive2(volatile uint8_t *rx_buf, uint16_t rx_num);
...
MD_STATUS U_IICA0_Master_Send_Wait(uint8_t adr7, const uint8_t *tx_buf, uint16_t tx_num)
{
    ...
    value = ulTaskNotifyTake_R_Helper_Ex( &g_iica0_master_task, status = R_IIC_Master_Send( (adr7 << 1), (uint8_t *)tx_buf, tx_num ) );
    if (MD_OK == status)
    {
        R_IIC_StopCondition();
...
MD_STATUS U_IICA0_Master_Receive_Wait(uint8_t adr7, uint8_t *rx_buf, uint16_t rx_num)
{
    ...
    /* Wait for a notification from the interrupt/callback */
    value = ulTaskNotifyTake_R_Helper_Ex( &g_iica0_master_task, status = R_IIC_Master_Receive( (adr7 << 1), rx_buf, rx_num ) );
    if (MD_OK == status)
    {
        R_IIC_StopCondition();
...
MD_STATUS U_IICA1_Slave_Send_Wait(const uint8_t *tx_buf, uint16_t tx_num)
{
    ...
    /* Wait for a notification from the interrupt/callback */
    value = ulTaskNotifyTake_R_Helper_Ex( &g_iica1_slave_task, U_IICA1_Slave_Send( tx_buf, tx_num ) );
...
MD_STATUS U_IICA1_Slave_Receive_Wait(uint8_t *rx_buf, uint16_t rx_num)
{
    ...
    /* Wait for a notification from the interrupt/callback */
    value = ulTaskNotifyTake_R_Helper_Ex( &g_iica1_slave_task, U_IICA1_Slave_Receive( rx_buf, rx_num ) );
...
MD_STATUS U_IICA1_Slave_Receive2_Wait(uint8_t *rx_buf, uint16_t rx_num)
{
    ...
    /* Wait for a notification from the interrupt/callback */
    value = ulTaskNotifyTake_R_Helper_Ex( &g_iica1_slave_task, U_IICA1_Slave_Receive2( rx_buf, rx_num ) );
