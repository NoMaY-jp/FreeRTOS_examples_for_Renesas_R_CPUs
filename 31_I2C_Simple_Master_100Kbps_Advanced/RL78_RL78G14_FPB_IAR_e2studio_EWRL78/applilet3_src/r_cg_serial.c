...
***********************************************************************************************************************/
uint8_t   g_iic00_master_status_flag;  /* iic00 start flag for send address check by master mode */
uint8_t * gp_iic00_tx_address;         /* iic00 send data pointer by master mode */
uint16_t  g_iic00_tx_count;            /* iic00 send data size by master mode */
uint8_t * gp_iic00_rx_address;         /* iic00 receive data pointer by master mode */
uint16_t  g_iic00_rx_count;            /* iic00 receive data size by master mode */
uint16_t  g_iic00_rx_length;           /* iic00 receive data length by master mode */
...
#define R_IIC00_Master_Send R_CG_API_DO_NOT_USE(R_IIC00_Master_Send)
#define R_IIC00_Master_Receive R_CG_API_DO_NOT_USE(R_IIC00_Master_Receive)
#define R_IIC00_Stop R_CG_API_DO_NOT_USE(R_IIC00_Stop)
#define R_IIC00_StartCondition R_CG_API_DO_NOT_USE(R_IIC00_StartCondition)
#define R_IIC00_StopCondition R_CG_API_DO_NOT_USE(R_IIC00_StopCondition)
...
MD_STATUS U_IIC00_Master_Send_Wait(uint8_t adr7, const uint8_t *tx_buf, uint16_t tx_num)
{
    ...
    value = ulTaskNotifyTake_R_Helper_Ex( &g_iic00_master_task, R_IIC_Master_Send( (adr7 << 1), (uint8_t *)tx_buf, tx_num ), portMAX_DELAY );
...
MD_STATUS U_IIC00_Master_Receive_Wait(uint8_t adr7, uint8_t *rx_buf, uint16_t rx_num)
{
    ...
    /* Wait for a notification from the interrupt/callback */
    value = ulTaskNotifyTake_R_Helper_Ex( &g_iic00_master_task, R_IIC_Master_Receive( (adr7 << 1), rx_buf, rx_num ), portMAX_DELAY );
...
/******************************************************************************
* Function Name: U_IICA0_Slave_Receive_Wait
* Description  : This function receives data as slave mode.
* Arguments    : rx_buf -
*                    receive buffer pointer
*                rx_num -
...
/******************************************************************************
* Function Name: U_IICA0_Slave_Receive2_Wait
